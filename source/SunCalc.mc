...
function durationToString(duration) {
		...
		var total_mins = (duration.value() + 30) / 60;
		var hours = total_mins / 60;
		var mins = total_mins % 60;
		
		var time = hours.format("%02d") + ":" + mins.format("%02d");
		return time;
	}
...
//			time = tinfo.hour.format("%02d") + ":" + tinfo.min.format("%02d") + ":" + tinfo.sec.format("%02d");
		} else {
...
*/
//			XM = getMeridiem(tinfo.hour);
			XM = (tinfo.hour < 12 || tinfo.hour == 24) ? "AM" : "PM";
		}
